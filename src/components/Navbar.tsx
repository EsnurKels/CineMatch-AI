...
export const Navbar = ({ isDarkMode, setIsDarkMode, activeTab, setActiveTab }: NavbarProps) => (
  <nav className={`fixed top-0 w-full z-50 border-b backdrop-blur-xl transition-all ${
    isDarkMode ? 'bg-[#0a0a0c]/80 border-slate-800' : 'bg-white/80 border-slate-200'
  }`}>
    <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
      <div className="flex items-center gap-10">
        <h1 onClick={() => setActiveTab('search')} className="text-2xl font-black tracking-tighter cursor-pointer flex items-center gap-2 group">
          <span className="bg-gradient-to-br from-indigo-400 to-cyan-400 bg-clip-text text-transparent">CINEMATCH</span>
        </h1>
